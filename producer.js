...
function get_total_data(){
  ...
  mysqlJson.query(`SELECT f2.* , s2.dep_time_utc, s2.dep_estimated_utc, s2.dep_actual_utc, s2.arr_time_utc, s2.arr_estimated_utc, s2.cs_flight_number, \
                  ...
                  s2.dep_time, s2.dep_actual_ts FROM ${tables[0]} f2 JOIN ${tables[2]} s2 ON f2.flight_number=s2.cs_flight_number UNION \
                  ...
                  s1.dep_time, s1.dep_actual_ts FROM ${tables[1]} f1 JOIN ${tables[3]} s1 ON f1.flight_number=s1.cs_flight_number`, function(err, response) {
    if (err) throw err;
